...
Applications should discover supported routes from the current API or SDK rather than copying a documentation table into product code.

```typescript
const sw4p = new Sw4p({ apiKey: process.env.SW4P_API_KEY })

const routes = await sw4p.getSupportedPairs()
const route = routes.find(
  (candidate) =>
    candidate.source === 'BASE' &&
    candidate.destination === 'SOLANA' &&
    candidate.asset === 'USDC'
)

if (!route || route.productionProofState !== 'ADMITTED') {
  throw new Error('Requested route is not currently admitted')
}
```

The exact SDK response types remain owned by the generated API reference.
